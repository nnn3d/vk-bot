...
class Api {
	...
	_worker () {
		...
		const worker = () => {
			...
			if (this.vk.options.call !== 'execute' || this._queue[0].method === 'execute' || this._queue[0].method === 'messages.setChatPhoto') {
				...
				for (let i = 0; i < this._queue.length; ++i) {
					if (this._queue[i].method === 'execute' || this._queue[0].method === 'messages.setChatPhoto') {
						continue;
					}
