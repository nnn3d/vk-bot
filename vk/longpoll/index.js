'use strict';


const Events = require('events');
const debug = require('debug')('vk-io:longpoll');

const RequestError = require('../errors/request');

const events = require('./events');
const { isRestartRequest } = require('../util/helpers');

/**
 * Работа с Longpoll
 *
 * @public
 */
class Longpoll extends Events {
    /**
     * Конструктор
     *
     * @param {VK} vk
     */
    constructor (vk) {
        super();

        this.vk = vk;
        this._launched = false;
        this._launchedGroups = false;
        this._restartsGroups = 0;
        this._restarts = 0;
        this._serverGroups = null;
        this._server = null;
        this._key = null;
        this._keyGroups = null
        this._ts = null;
        this._tsGroups = null;
        this._pts = 0;
        this._ptsGroups = 0;

        /**
         * 2 - Прикрипления
         * 64 - Идентификатор платформы user.online
         *
         * @type {number}
         */
        this._mode = 2 + 64;
        //console.log(vk.options.token.slice(0, 8));
        //console.log(this.vk.options.token.slice(0, 8));
        this.on('error', (error) => {
            debug('error', error);

            setTimeout(() => {
                if (!this._launched) {
                    return;
                }
                console.log("on1",this.vk.options.token);
                this.restart()
                    .catch((error) => {
                        this.emit('error', error);
                    });
            }, this.vk.options.longpollWait);});
        this.on('error', (error) => {
            debug('error', error);
            setTimeout(() => {
                if (!this._launchedGroups) {
                    return;
                }
                console.log("on2",this.vk.options.token);
                this.restartGroups()
                    .catch((error) => {
                        this.emit('error', error);
                    });
            }, this.vk.options.longpollWait);
        });
    }

    /**
     * Запущен ли longpoll
     *
     * @return {boolean}
     */
    isStarted () {
        return this._launched;
    }

    /**
     * Запущен ли longpoll
     *
     * @return {boolean}
     */
    isStartedGroups () {
        return this._launchedGroups;
    }

    /**
     * Запускает longpoll
     *
     * @return {Promise<void>}
     */
    start () {
        if (this._launched) {
            return Promise.resolve();
        }

        debug('started');

        this._launched = true;
        return this.vk.api.messages.getLongPollServer()
            .then(({ server, key, ts }) => {
                //console.log(this.vk.options.token.slice(0, 8));
                this._server = `https://${server}`;
                this._key = key;

                if (this._ts === null) {
                    this._ts = ts;
                }

                this._loop();
            })
            .catch((error) => {
                this._launched = false;
                console.log('longpoll start error');
                return new Promise(resolve => setTimeout(resolve, 3e3))
                    .then(() => this.start());
                //
                // throw error;
            });
    }

    /**
     * Запускает longpoll
     *
     * @return {Promise<void>}
     */
    startGroups() {
        if (this._launchedGroups) {
            return Promise.resolve();
        }

        debug('started');

        this._launchedGroups = true;
        return this.vk.api.groups.getLongPollServer({group_id: Math.abs(this.vk.selfId)})
            .then(({ server, key, ts }) => {
             //   console.log(this.vk.options.token.slice(0, 8));
                this._serverGroups = `${server}`;
                this._keyGroups = key;

                if (this._tsGroups === null) {
                    this._tsGroups = ts;
                }

                this._loopGroups();
            })
            .catch((error) => {
                console.error(error);
                this._launchedGroups = false;
                console.log('longpoll groups start error');
                return new Promise(resolve => setTimeout(resolve, 3e3))
                    .then(() => this.startGroups());
                //
                // throw error;
            });
    }

    /**
     * Останавливает работу longpoll
     *
     * @return {Promise<void>}
     */
    stop () {
        debug('stoped');

        this._ts = null;
        this._restarts = 0;
        this._launched = false;

        return Promise.resolve();
    }

    /**
     * Останавливает работу longpoll
     *
     * @return {Promise<void>}
     */
    stopGroups () {
        debug('stoped');

        this._tsGroups = null;
        this._restartsGroups = 0;
        this._launchedGroups = false;

        return Promise.resolve();
    }

    /**
     * Перезапускает longpoll
     *
     * @return {Promise<void>}
     */
    restart () {
        debug('restart');

        this._restarts = 0;
        this._launched = false;

        return this.start();
    }

    /**
     * Перезапускает longpoll
     *
     * @return {Promise<void>}
     */

    restartGroups () {
        debug('restart');

        this._restartsGroups = 0;
        this._launchedGroups = false;

        return this.startGroups();
    }

    /**
     * Генерировать ли событие pts
     *
     * @param {boolean} need
     *
     * @return {this}
     */
    usePts (need = true) {
        if (need && (this._mode & 32) === 0) {
            this._mode += 32;
        } else if (!need && (this._mode & 32) === 32) {
            this._mode -= 32;
        }

        return this;
    }

    /**
     * Цикличное получение обновлений
     */
    _loop () {
        debug('http -->');

        const startTime = Date.now();

        new Promise((resolve, reject) => {
            this.vk.request({
                uri: this._server,
                timeout: 3e3,
                qs: {
                    mode: this._mode,
                    key: this._key,
                    act: 'a_check',
                    ts: this._ts,
                    version: 2,
                    wait: 20
                }
            }).then(resolve);
            setTimeout(reject, 3e3);
        })
            .then((response) => {
                this._restarts = 0;

                const endTime = (Date.now() - startTime).toLocaleString();

                debug(`http <-- ${endTime}ms`);

                if ('failed' in response && response.failed !== 1) {
                    response._ts = null;

                    debug('failed 1');

                    return void this.restart()
                        .catch((error) => {
                            this.emit('error', error);
                        });
                }

                this._ts = +response.ts;

                if ('pts' in response && response.pts !== this._pts) {
                    this.emit('pts', {
                        ts: this._ts,
                        pts: this._pts = +response.pts
                    });
                }

                if ('updates' in response) {
                    // console.log("Профиль:");
                    //  console.log(response.updates[0]);
                    for (const update of response.updates) {
                        this._restructure(update);
                    }
                }

                if (this._launched) {
                    return void this._loop();
                }
            })
            .catch((error) => {
                if (!this._launched) {
                    return;
                }

                const { longpollCount, longpollWait } = this.vk.options;

                if (this._restarts < longpollCount) {
                    ++this._restarts;

                    debug(`restarted ${this._restarts} times`);

                    return setTimeout(() => {
                        this._loop();
                    }, longpollWait);
                }

                this.restart()
                    .catch((error) => {
                        this.emit('error', error);
                    });
            });
    }

    /**
     * Цикличное получение обновлений
     */
    _loopGroups() {
        debug('http -->');

        const startTime = Date.now();

        new Promise((resolve, reject) => {
            this.vk.request({
                uri: this._serverGroups,
                timeout: 3e3,
                qs: {
                    key: this._keyGroups,
                    ts: this._tsGroups,
                    act: 'a_check',
                    wait: 20
                }
            }).then(resolve);
            setTimeout(reject, 3e3);
        })
            .then((response) => {
                this._restartsGroups = 0;
                const endTime = (Date.now() - startTime).toLocaleString();

                debug(`http <-- ${endTime}ms`);

                if ('failed' in response && response.failed !== 1) {
                    response._tsGroups = null;

                    debug('failed 1');

                    return void this.restartGroups()
                        .catch((error) => {
                            this.emit('error', error);
                        });
                }

                this._tsGroups = +response.ts;

                if ('updates' in response) {
                    //console.log("Группа:");
                    // console.log(response.updates);
                    for (const update of response.updates) {
                        this._restructureGroups(update);
                    }
                }

                if (this._launchedGroups) {
                    return void this._loopGroups();
                }
            })
            .catch((error) => {
                if (!this._launchedGroups) {
                    return;
                }

                const { longpollCount, longpollWait } = this.vk.options;

                if (this._restartsGroups < longpollCount) {
                    ++this._restartsGroups;

                    debug(`restarted ${this._restartsGroups} times`);

                    return setTimeout(() => {
                        this._loopGroups();
                    }, longpollWait);
                }

                this.restartGroups()
                    .catch((error) => {
                        this.emit('error', error);
                    });
            });
    }

    /**
     * Рестрирует данные обновления
     *
     * @param {Array} update
     */
    _restructure (update) {
        this.emit('raw', update);
        const id = update[0];

        if (!(id in events)) {
            return;
        }
        const { name, action } = events[id];

        if (id !== 4 && this.listenerCount(name) === 0) {
            return;
        }

        let result;

        try {
            result = action.call(this, update);
        } catch (error) {
            debug('longpoll handler parse error', error);

            return;
        }

        if (result === null) {
            return;
        }

        try {
            if (Array.isArray(result)) {
                return this.emit(result[1], result[0]);
            }

            this.emit(name, result);
        } catch (error) {
            console.error(error);
        }
    }


    /**
     * Рестрирует данные обновления
     *
     * @param {Array} update
     */
    _restructureGroups (update) {
        this.emit('raw', update);
        const type = update.type;
        if (type !== 'message_new') {
            return;
        }

        const { name, action } = events[type];

        if (this.listenerCount(name) === 0) {
            return;
        }
        let result;

        try {
            result = action.call(this, update.object);
        } catch (error) {
            debug('longpoll handler parse error', error);
            console.error(error);
            return;
        }

        if (result === null) {
            return;
        }

        try {
            if (Array.isArray(result)) {
                return this.emit(result[1], result[0]);
            }

            this.emit(name, result);
        } catch (error) {
            console.error(error);
        }
    }
}

module.exports = Longpoll;
