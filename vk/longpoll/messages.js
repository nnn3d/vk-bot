'use strict';

const { inspect } = require('util');

const { SHEAR_CHAT_PEER } = require('../util/constants');
const {
	parseFlags,
	parseAttachments,
	parseFwds,
	brReplace,
	unescape
} = require('./helpers');

/**
 * Базовай класс сообщений ВКонтакте
 *
 * @private
 */
class BaseMessage {
	/**
	 * Конструктор
	 *
	 * @param {VK}    vk
	 * @param {Array} message
	 */
	constructor (vk, message, fromGroup) {
		this.vk = vk;
		if (fromGroup) {
			this.id = +message.id || +message.conversation_message_id

			this.date = +message.date;
			this.peer = +message.peer_id;
		} else {
            this.id = +message[1];

            this.date = +message[4];
            this.peer = +message[3];
		}
	}

	/**
	 * Отправляет сообщение
	 *
	 * @param {mixed}  text
	 * @param {Object} params
	 *
	 * @return {Promise<Number>}
	 */
	send (text, params = {}) {
		if (typeof text === 'object') {
			params = text;
		} else {
			params.message = text;
		}

		params.peer_id = this.peer;

		return this.vk.api.messages.send(params);
	}

	/**
	 * Отправляет стикер в текущий диалог
	 *
	 * @param {number} id
	 *
	 * @return {Promise<Number>}
	 */
	sendSticker (id) {
		return this.send({
			sticker_id: id
		});
	}

	/**
	 * Отправляет фотографию в диалог
	 *
	 * @param {mixed}  source
	 * @param {Object} params
	 *
	 * @return {Promise<Number>}
	 */
	sendPhoto (source, params = {}) {
		return this.vk.upload.message({
			source
		})
		.then((photo) => (
			this.vk.getAttachment('photo', photo)
		))
		.then((attachment) => (
			this.send(Object.assign(params, { attachment }))
		));
	}

	/**
	 * Изменяет статус набора текста пользователем в диалоге
	 *
	 * @param {Promise<void>}
	 */
	setActivity () {
		return this.vk.api.messages.setActivity({
			type: 'typing',
			peer_id: this.peer
		});
	}

	/**
	 * Возвращает свойства которые нужно вывести
	 *
	 * @param {number} depth
	 * @param {Object} options
	 *
	 * @return {Object}
	 */
	inspect (depth, options) {
		return {
			id: this.id,
			date: this.date,
			peer: this.peer
		};
	}
}

/**
 * Базовый класс для всех событий чата
 *
 * @private
 */
class ChatEvent extends BaseMessage {
	/**
	 * Конструктор
	 *
	 * @param {VK}    vk
	 * @param {Array} message
	 */
	constructor (vk, message, fromGroup) {
		super(vk, message, fromGroup);

		if (fromGroup) {
			this.user = +message.from_id
		} else {
			this.user = +message[6].from;
		}

        this.chat = this.peer - SHEAR_CHAT_PEER;
	}

	/**
	 * Возвращает свойства которые нужно вывести
	 *
	 * @param  {number} depth
	 * @param  {Object} options
	 *
	 * @return {Object}
	 */
	inspect (depth, options) {
		return Object.assign(super.inspect(depth, options), {
			user: this.user,
			chat: this.chat
		});
	}
}

/**
 * Сообщение
 *
 * @public
 */
class Message extends BaseMessage {
	/**
	 * Конструктор
	 *
	 * @param {VK}     vk
	 * @param {Object} message
	 * @param {Boolean} fromGroup
	 */
	constructor (vk, message, fromGroup) {
		super(vk, message, fromGroup);

		if (fromGroup) {
            if (this.peer > SHEAR_CHAT_PEER) {
                this.user = +message.from_id;

                this.chat = this.peer - SHEAR_CHAT_PEER;

                this.from = 'chat';
            } else if (this.peer < 0) {
                this.admin = +message.from_admin;

                this.from = 'group';
            } else {
                this.user = this.peer;

                this.from = 'dm';
            }

            if (message.text) {
                this.text = unescape(message.text).replace(brReplace, '\n');
            } else {
                this.text = null;
            }

            this.flags = {}
            this.attachments = {};

            if (message.attachments) {
                message.attachments.map((attach) => {
                    this.attachments[attach.type] = this.attachments[attach.type] || []
                    this.attachments[attach.type].push(attach)
                })
            }

            this.hasEmoji = 'emoji' in this.attachments;

            this._fwd = message.fwd_messages || null;
		} else {
            const attachments = message[6];

            if (this.peer > SHEAR_CHAT_PEER) {
                this.user = +attachments.from;

                this.chat = this.peer - SHEAR_CHAT_PEER;

                this.from = 'chat';
            } else if (this.peer < 0) {
                this.admin = +attachments.from_admin;

                this.from = 'group';
            } else {
                this.user = this.peer;

                this.from = 'dm';
            }

            if (message[5].length !== 0) {
                this.text = unescape(message[5]).replace(brReplace, '\n');
            } else {
                this.text = null;
            }

            this.flags = parseFlags(message[2], this.isGroup());
            this.attachments = parseAttachments(attachments);

            this.hasEmoji = 'emoji' in attachments;

            if ('geo' in attachments) {
                this.attachments.geo = {
                    id: attachments.geo,
                    provider: +attachments.geo_provider
                };
            }

            this._fwd = attachments.fwd || null;
		}
	}

	/**
	 * Отвечает на сообщение
	 *
	 * @param {mixed}  text
	 * @param {Object} params
	 *
	 * @return {Promise<Number>}
	 */
	reply (text, params = {}) {
		if (typeof text === 'object') {
			params = text;
		} else {
			params.message = text;
		}

		params.forward_messages = this.id;

		return this.send(params);
	}

	/**
	 * Проверяет наличие флага
	 *
	 * @param {string} name
	 *
	 * @return {boolean}
	 */
	hasFlag (name) {
		return this.flags.includes(name);
	}

	/**
	 * Проверяет наличие прикриплений
	 *
	 * @return {boolean}
	 */
	hasAttachments () {
		return Object.keys(this.attachments).length > 0;
	}

	/**
	 * Проверяет наличие прикрипления
	 *
	 * @param {string} name
	 *
	 * @return {boolean}
	 */
	hasAttachment (name) {
		return name in this.attachments;
	}

	/**
	 * Проверяет наличие пересылаемых сообщений
	 *
	 * @return {boolean}
	 */
	hasFwd () {
		return this._fwd !== null;
	}

	/**
	 * Сообщение из диалога
	 *
	 * @return {boolean}
	 */
	isDM () {
		return this.from === 'dm';
	}

	/**
	 * Сообщение из беседы
	 *
	 * @return {boolean}
	 */
	isChat () {
		return this.from === 'chat';
	}

	/**
	 * Сообщения из сообщества
	 *
	 * @return {boolean}
	 */
	isGroup () {
		return this.from === 'group';
	}

	/**
	 * Возвращает пересылаемые сообщения
	 *
	 * @return {Array}
	 */
	getFwd () {
		if (!this.hasFwd()) {
			return [];
		}

		if (Array.isArray(this._fwd)) {
			return this._fwd;
		}

		this._fwd = parseFwds(this._fwd);

		return this._fwd;
	}

	/**
	 * Возвращает свойства которые нужно вывести
	 *
	 * @return object
	 */
	inspect (depth, options) {
		const print = Object.assign(super.inspect(depth, options), {
			user: this.user,
			chat: this.chat,
			text: this.text,
			from: this.from,
			hasEmoji: this.hasEmoji,
			flags: this.flags,
			attachments: this.attachments
		});

		return this.constructor.name + ' ' + inspect(print, options);
	}
}

exports.Message = Message;

/**
 * Создание чата
 *
 * @public
 */
class ChatCreate extends ChatEvent {
	/**
	 * Конструктор
	 *
	 * @param {VK}    vk
	 * @param {Array} message
	 */
	constructor (vk, message, fromGroup) {
		super(vk, message, fromGroup);
	}

	/**
	 * Возвращает свойства которые нужно вывести
	 *
	 * @param  {number} depth
	 * @param  {Object} options
	 *
	 * @return {Object}
	 */
	inspect (depth, options) {
		return this.constructor.name + ' ' + inspect(
			super.inspect(depth, options),
			options
		);
	}
}

/**
 * Обновлено название беседы
 *
 * @public
 */
class TitleUpdate extends ChatEvent {
	/**
	 * Конструктор
	 *
	 * @param {VK}    vk
	 * @param {Array} message
	 */
	constructor (vk, message, fromGroup) {
		super(vk, message, fromGroup);
		if (fromGroup) {
			this.title = unescape(message.action.text)
		} else {
            this.title = unescape(message[6].source_text);
        }
	}

	/**
	 * Обновляет название беседы
	 *
	 * @param {string} title
	 *
	 * @return {Promise<void>}
	 */
	rename (title) {
		return this.vk.api.messages.editChat({
			chat_id: this.chat,
			title
		});
	}

	/**
	 * Возвращает свойства которые нужно вывести
	 *
	 * @param {number} depth
	 * @param {Object} options
	 *
	 * @return {Object}
	 */
	inspect (depth, options) {
		return this.constructor.name + ' ' + inspect(
			super.inspect(depth, options),
			options
		);
	}
}

class PinUpdate extends ChatEvent {
    /**
     * Конструктор
     *
     * @param {VK}    vk
     * @param {Array} message
     */
    constructor (vk, message, fromGroup) {
        super(vk, message, fromGroup);

        if (fromGroup) {
        	this.pin = unescape(message.action.conversation_message_id)

			this.text = message.action.message || ''
		} else {

            this.pin = unescape(message[6].source_chat_local_id);

            this.text = '';
            if (message[6].source_message) {
                this.text = message[6].source_message;
            }
        }
    }

    /**
     * Возвращает свойства которые нужно вывести
     *
     * @param {number} depth
     * @param {Object} options
     *
     * @return {Object}
     */
    inspect (depth, options) {
        return this.constructor.name + ' ' + inspect(
            super.inspect(depth, options),
            options
        );
    }
}

/**
 * Обновлена фотография беседы
 *
 * @public
 */
class PhotoUpdate extends ChatEvent {
	/**
	 * Конструктор
	 *
	 * @param {VK}    vk
	 * @param {Array} message
	 */
	constructor (vk, message, fromGroup) {
		super(vk, message, fromGroup);

		if (fromGroup) {
			const { id, owner_id } = message.attachments[0] && message.attachments[0].photo || {}
			this.photo = {
				id: id,
				owner: owner_id
			}
		} else {
            const [onwer, id] = message[6].attach1.split('_');

            this.photo = {
                id: +id,
                owner: +onwer
            };
		}
	}

	/**
	 * Удаляет фотографию беседы
	 *
	 * @return {Promise<void>}
	 */
	remove () {
		return this.vk.api.messages.deleteChatPhoto({
			chat_id: this.chat
		});
	}

	/**
	 * Возвращает свойства которые нужно вывести
	 *
	 * @param {number} depth
	 * @param {Object} options
	 *
	 * @return {Object}
	 */
	inspect (depth, options) {
		const print = Object.assign(super.inspect(depth, options), {
			photo: this.photo
		});

		return this.constructor.name + ' ' + inspect(print, options);
	}
}

/**
 * Удаление фотографии беседы
 */
class PhotoRemove extends ChatEvent {
	/**
	 * Возвращает свойства которые нужно вывести
	 *
	 * @param {number} depth
	 * @param {Object} options
	 *
	 * @return {string}
	 */
	inspect (depth, options) {
		return this.constructor.name + ' ' + inspect(
			super.inspect(depth, options),
			options
		);
	}
}

/**
 * Добавлен пользователь в беседу
 */
class InviteUser extends ChatEvent {
	/**
	 * Конструктор
	 *
	 * @param {VK}    vk
	 * @param {Array} message
	 */
	constructor (vk, message, fromGroup) {
		super(vk, message, fromGroup);

		if (fromGroup) {
			this.invite = +message.action.member_id || this.user;
            this.link = !message.action.member_id;
		} else {
            this.invite = +message[6].source_mid || this.user;
            this.link = !message[6].source_mid;
		}
	}

	/**
	 * Исключает пользователя
	 *
	 * @param {number} id
	 *
	 * @return {Promise<void>}
	 */
	kick (id = this.invite) {
		return this.vk.api.messages.removeChatUser({
			chat_id: this.chat,
			user_id: id
		});
	}

	/**
	 * Возвращает свойства которые нужно вывести
	 *
	 * @param {number} depth
	 * @param {Object} options
	 *
	 * @return {Object}
	 */
	inspect (depth, options) {
		const print = Object.assign(super.inspect(depth, options), {
			invite: this.invite
		});

		return this.constructor.name + ' ' + inspect(print, options);
	}
}

/**
 * Исключен пользователь из беседы
 */
class KickUser extends ChatEvent {
	/**
	 * Конструктор
	 *
	 * @param {VK}    vk
	 * @param {Array} message
	 */
	constructor (vk, message, fromGroup) {
		super(vk, message, fromGroup);

		if (fromGroup) {
			this.kick = +message.action.member_id;
		} else {
            this.kick = +message[6].source_mid;
        }
	}

	/**
	 * Приглашает пользователя обратко
	 *
	 * @param {number} id
	 *
	 * @return {Promise<void>}
	 */
	invite (id = this.kick) {
		return this.vk.api.messages.addChatUser({
			chat_id: this.chat,
			user_id: id
		});
	}

	/**
	 * Возвращает свойства которые нужно вывести
	 *
	 * @param {number} depth
	 * @param {Object} options
	 *
	 * @return {Object}
	 */
	inspect (depth, options) {
		const print = Object.assign(super.inspect(depth, options), {
			kick: this.kick
		});

		return this.constructor.name + ' ' + inspect(print, options);
	}
}

/**
 * Обработчики событий чата
 */
exports.actions = {
	chat_create: (vk, message, fromGroup) => (
		[new ChatCreate(vk, message, fromGroup), 'chat.create']
	),

	chat_title_update: (vk, message, fromGroup) => (
		[new TitleUpdate(vk, message, fromGroup), 'chat.rename']
	),

	chat_photo_update: (vk, message, fromGroup) => (
		[new PhotoUpdate(vk, message, fromGroup), 'chat.photo.update']
	),
	/* Это не стандартизировано */
	chat_photo_remove: (vk, message, fromGroup) => (
		[new PhotoRemove(vk, message, fromGroup), 'chat.photo.remove']
	),

	chat_pin_message: (vk, message, fromGroup) => (
		[new PinUpdate(vk, message, fromGroup), 'chat.pin']
	),

	chat_unpin_message: (vk, message, fromGroup) => (
		[new PinUpdate(vk, message, fromGroup), 'chat.unpin']
	),

	chat_invite_user: (vk, message, fromGroup) => (
		[new InviteUser(vk, message, fromGroup), 'chat.invite']
	),

    chat_invite_user_by_link: (vk, message, fromGroup) => (
		[new InviteUser(vk, message, fromGroup), 'chat.invite']
	),

	chat_kick_user: (vk, message, fromGroup) => (
		[new KickUser(vk, message, fromGroup), 'chat.kick']
	)
};
